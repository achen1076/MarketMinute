...
class CuratedFeatureEngine:
    ...
    def calculate_all(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"Curated features complete: {len(df.columns)} total columns")
        return df
    ...
    def _add_volatility(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['parkinson_vol'] = np.sqrt((1 / (4 * np.log(2))) * (np.log(hl_ratio) ** 2))
    ...
    def _add_microstructure(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Price impact proxy
        df['price_impact'] = abs(df['ret_1']) / (np.log(df['volume'] + 1) + 1e-8)
